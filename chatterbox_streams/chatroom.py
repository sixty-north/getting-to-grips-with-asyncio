...
class ChatRoom:
    ...
    async def client_connected(self, reader, writer):
        writeline(writer, "Welcome to {}".format(self.name))
    ...
    def user_departed(self, registered_user):
        self.broadcast("User {} departed.".format(registered_user))

    def user_arrived(self, registered_user):
        self.broadcast("User {} arrived.".format(registered_user))
    ...
    def register_user(self, username, reader, writer):
        if username in self.users():
            writeline(writer, "Username {} not available".format(username))
            return False
    ...
    def deregister_user(self, username):
        ...
        self.broadcast("User {} departed.".format(username))

    def list_users(self, writer):
        writeline(writer, "Users here:")
        for username in self.users():
            writeline(writer, "  {}".format(username))
